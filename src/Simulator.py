...
class ChessSimulation:
    ...
    def play_game(self, white, black):
        ...
        while not board.is_game_over():
            ...
            score = evaluation["score"].white().score(mate_score=10000)  # Convert to a consistent perspective
    ...
    def get_move(self, player, board):
        if isinstance(player, AlphaPawn):
            ...
            result = self.engine.play(board, chess.engine.Limit(time=0.1))  # Reduced time for faster simulation
            return result.move
    ...
    def write_to_csv(self, file_name):
        with open(file_name, mode='w', newline='', encoding='utf-8') as file:
            writer = csv.DictWriter(file, fieldnames=["winner", "loser", "white_pieces", "black_pieces", "moves", "evaluations"])
            writer.writeheader()
